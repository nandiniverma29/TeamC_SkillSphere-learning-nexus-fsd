...
export default function InstructorDashboard() {
  ...
  const load = async () => {
    try {
      const [ovRes, coursesRes] = await Promise.all([
        fetch("http://localhost:8080/api/instructor/overview", { headers }),
        fetch("http://localhost:8080/api/instructor/courses", { headers }),
      ]);
      if (!ovRes.ok || !coursesRes.ok) {
        const data = await (ovRes.ok ? coursesRes : ovRes).json();
        throw new Error(data.error || "Failed to load instructor data");
      }
  ...
  const togglePublished = async (course) => {
    try {
      const res = await fetch(`http://localhost:8080/api/instructor/courses/${course.id}`, {
        method: "PUT",
        headers,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingId
        ? `http://localhost:8080/api/instructor/courses/${editingId}`
