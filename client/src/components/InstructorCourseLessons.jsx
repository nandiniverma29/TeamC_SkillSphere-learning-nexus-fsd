...
export default function InstructorCourseLessons() {
  ...
  const load = async () => {
    try {
      const res = await fetch(`http://localhost:8080/api/instructor/courses/${courseId}/lessons`, { headers });
  ...
  const handleAddLesson = async (e) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`http://localhost:8080/api/instructor/courses/${courseId}/lessons`, {
        method: "POST",
        headers,
        body: JSON.stringify(lessonForm),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Could not create lesson");
  ...
  const handleDeleteLesson = async (lesson) => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/instructor/lessons/${lesson.id}`, { method: "DELETE", headers });
      const data = await res.json();
  ...
  const handleAddQuestion = async (e, lessonId) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`http://localhost:8080/api/instructor/lessons/${lessonId}/questions`, {
        method: "POST",
        headers,
  ...
  const handleDeleteQuestion = async (questionId) => {
    try {
      const res = await fetch(`http://localhost:8080/api/instructor/questions/${questionId}`, { method: "DELETE", headers });
      const data = await res.json();
