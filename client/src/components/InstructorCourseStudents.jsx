...
export default function InstructorCourseStudents() {
  ...
  const loadAnnouncements = async () => {
    try {
      const res = await fetch(`http://localhost:8080/api/instructor/courses/${courseId}/announcements`, { headers });
  ...
  const handlePost = async (e) => {
    ...
    setPosting(true);
    try {
      const res = await fetch(`http://localhost:8080/api/instructor/courses/${courseId}/announcements`, {
        method: "POST",
        headers,
        body: JSON.stringify({ message }),
      });
      const body = await res.json();
      if (!res.ok) throw new Error(body.error || "Could not post announcement");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`http://localhost:8080/api/instructor/courses/${courseId}/students`, {
